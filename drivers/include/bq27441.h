 ...
 * @brief       Device driver interface for the BQ27441 fuel gauge for
 *              single-cell Li-Ion batteries
 * @{
 *
 * @file
 * @brief       Device driver interface for the BQ27441 fuel gauge
 *
...
/**
 * @brief   Parameters for the @ref bq27441_get_current() function, to specify
 *          which current to read
 * @{
 */
typedef enum {
...
/**
 * @brief   Parameters for the @ref bq27441_get_capacity() function, to specify
 *          which current to read
 * @{
 */
typedef enum {
...
/**
 * @brief   Parameters for the @ref bq27441_get_soc() function
 * @{
 */
typedef enum {
...
/**
 * @brief   Parameters for the @ref bq27441_get_soh() function
 * @{
 */
typedef enum {
...
/**
 * @brief   Parameters for the @ref bq27441_get_temperature() function
 * @{
 */
typedef enum {
...
/**
 * @brief   Parameters for the @ref bq27441_set_gpout_function() function
 * @{
 */
typedef enum {
 ...
 *              triggered on low RSOC or Voltage
 *
 ...
 * @details     This struct will hold all information and configuration for the
 *              sensor
 * @{
 */
typedef struct {
 ...
 *
 * @param[in]   *dev        pointer to bq27441_t struct containing the i2c
 *                          device and the address
 * @param[in]   *params     pointer to bq27441_params_t struct containing the
 *                          interrupt pin and callback
 *
 ...
 * @param[in]   *dev        pointer to bq27441_t struct containing the i2c
 *                          device and the address
 * @param[in]   capacity    capacity of battery in mAh (unsigned 16-bit value)
 *
 ...
 *
 * @return              specified current measurement in mA. >0 indicates
 *                      charging.
 */
 ...
 *                      and the address
 *
 * @return              average power in Wh. >0 indicates charging.
 */
 ...
 * @param[in]   type    enum specifying filtered or unfiltered measurement
 *
 * @return              specified state of health measurement in %, or status
 *                      bits
 */
...
/**
 * @brief       Enter configuration mode - set userControl if calling from an
 *              Arduino sketch and you want control over when to exitConfig.
 *
 * @param[in]   *dev            pointer to bq27441_t struct containing the i2c
 *                              device and the address
 * @param[in]   userControl     true if the Arduino sketch is handling entering
 *                              and exiting config mode (should be false in
 *                              library calls).
 *
 * @return                      true on success
 */
 ...
 * @return              true on success
 */
bool _bq27441_write_block_data(const bq27441_t *dev, uint8_t offset, uint8_t
                               data);
 ...
 * @param[in]   *dev    pointer to bq27441_t struct containing the i2c device
 *                      and the address
 * @param[in]   csum    8-bit checksum to be written
 *
 * @return              true on success
 */
...
/**
 * @brief       Read a byte from extended data specifying a class ID and
 *              position offset
 *
 * @param[in]   *dev        pointer to bq27441_t struct containing the i2c
 *                          device and the address
